...
{
  bool check;
  if((depChairId == !0) && (!DeptChairExist(depChairId)))    //If the given department chair id does not exist, return failure.
    {
...
bool University::CreateNewCourse(string cName, long cDepId, long cTaughtBy, int cMaxSeat)
{
  ...
  if(cTaughtBy == 0)                                     //If the course is taught by no one, then return success.
    {
